...
docker save rabbitmq:4.2.5-management -o /tmp/rabbitmq.tar
...
scp /tmp/rabbitmq.tar $SERVER:$DEPLOY_PATH/images/
...
ssh $SERVER "cd $DEPLOY_PATH && \
    ...
    rm -f images/*.tar && \
    echo '创建数据目录...' && \
    mkdir -p data/postgres data/rabbitmq data/chromadb && \
    echo '启动服务...' && \
    cd docker && docker compose up -d postgres rabbitmq chromadb && \
    echo '等待服务启动...' && \
    sleep 10 && \
    echo '服务状态:' && \
    docker compose ps"
...
echo ">>> 清理本地临时文件..."
rm -f /tmp/postgres.tar /tmp/rabbitmq.tar /tmp/chromadb.tar
